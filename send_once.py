...
POLLING_DELAY = 1.5
CURRENCY_CACHE = {}
# Cotação de emergência caso todas APIs falhem - atualize manualmente 1x por semana
...
def get_exchange_rate(from_currency: str, to_currency: str = "BRL") -> Optional[float]:
    ...
    # 3 APIs em ordem de preferência
    apis = [
    ...
    for api in apis:
        try:
            req = urllib.request.Request(api["url"], headers={"User-Agent": "DuffelMonitor/v20.8"})
            with urllib.request.urlopen(req, timeout=8) as response:
                raw = response.read().decode("utf-8")
                # Log dos primeiros 100 chars pra debug se der erro
    ...
    # Se tudo falhar, usa cotação de emergência
    ...
    logger.error(f"CURRENCY_FAIL from={from_currency} to={to_currency} reason=all_apis_failed_no_emergency")
    return None
...
class DuffelClient:
    ...
    def _try_version(self, version: str, endpoint: str, method: str, body: Optional[Dict]) -> Tuple[Optional[Dict], Optional[str]]:
        ...
        headers = {
            ...
            "User-Agent": "DuffelMonitor/v20.8"
        }
...
def send_email(azul: Dict, gol: Dict) -> bool:
    try:
        ...
        <h2>✈️ MELHORES PREÇOS - DUFFEL EDITION (v20.8)</h2>
        ...
        <p><small>Cotação automática em {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}</small></p>
        </body></html>
        """
...
def send_telegram(azul: Dict, gol: Dict) -> bool:
    try:
        ...
        msg_text = f"""✈️ MELHORES PREÇOS - DUFFEL EDITION (v20.8)
...
def main():
    logger.info("=" * 60)
    logger.info("🚀 Script v20.8 - Duffel Edition SEGURA")
    logger.info("=" * 60)
